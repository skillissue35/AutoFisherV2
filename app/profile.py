    ...
    def update(self, raw_data: str) -> bool:
        ...
        for line in remove_markdown(raw_data).split('\n'):
            ...
            #Exotic fishes
                ...
                self.exotic_fish.gold = int(sub("[^0-9]", "", line))
            elif line.find('Emerald Fish') > -1:
                end = line.find('<')
                self.exotic_fish.emerald = int(sub("[^0-9]", "", line))
            elif line.find('Lava Fish') > -1:
                end = line.find('<')
                self.exotic_fish.lava = int(sub("[^0-9]", "", line))
            elif line.find('Diamond Fish') > -1:
                end = line.find('<')
                self.exotic_fish.diamond = int(sub("[^0-9]", "", line))
